    ...
    def __validate_user_resource_request_info(self, group_info, group_info_db, resource_request_type, request_type):
        ...
        if request_type == 'CREATE':
            ...
            if resource_request_type in group_info_db['resources'] and len(group_info_db['resources'][resource_request_type]) > 0 and resource_request_id in group_info_db['resources'][resource_request_type]:
                raise Exception('Cannot process the resource request: The resource request with same request name already in system')
  
        return True
    
    def __transfer_user_resource_request_info(self, group_info, group_info_db, resource_request_type, request_type):
        if self.__validate_user_resource_request_info(user_resource_request_info, group_info_db, resource_request_type, request_type):
            if request_type == 'CREATE':
                group_info_db['pi_uid'] = user_resource_request_info['pi_uid']
                group_info_db['project_name'] = user_resource_request_info['project_name']
                group_info_db['project_desc'] = user_resource_request_info['project_desc']
                group_info_db['data_agreement_signed'] = user_resource_request_info['data_agreement_signed']
                group_info_db['delegates_uid'] = user_resource_request_info['delegates_uid']
                resource_request_id = group_info['group_name'] + group_info['resources'][resource_request_type][group_info['group_name']]['tier']
